...
ScriptIDF = function(ArquivPrec){
  ...
  #Identifica a distribuição probabilística que melhor se ajusta a série histórica
  ...
  if(dfTAR > valcrit){
    ...
    #Determina as intensidades máximas observadas a partir da distribuição probabilística que melhor se ajustou a série histórica
    ...
    #Otimiza os coeficientes das equações de chuvas intensas
    ...
    Saida = function(TamanhoArquivDuracoes,TamanhoArquivTr,kotim,ArquivTr,motim,ArquivDuracoes,t0otim,notim,ArquivPrec,IMaxObs,dfTAD){
      ...
      AnoSerieInicio <- min(ArquivPrec$Ano)
      AnoSerieFim <- max(ArquivPrec$Ano)
      
      TamanhoSerie <- c((AnoSerieFim-AnoSerieInicio)+1)
      
      MaxPrec <- c(max(ArquivPrec$Prec))
      ...
      TabFinal <- data.frame(AnoSerieInicio,AnoSerieFim,TamanhoSerie,MaxPrec,K,m,t0,n,Distribuicao,
                             NS,R2,RMSE,MAE) %>%
        setNames(.,c("Beginning of the historical series","End of the historical series","Historical series size",
                     "Maximum daily precipitation","Coefficient - K","Coefficient - m","Coefficient - t0",
                     "Coefficient - n","Probability distribution","Statistic - NS","Statistic - R2",
    ...
    TabelaResultados <- Saida(TamanhoArquivDuracoes,TamanhoArquivTr,kotim,ArquivTr,motim,ArquivDuracoes,t0otim,notim,ArquivPrec,IMaxObs,dfTAD)$TabFinal
    
    View(TabelaResultados)
    
    #Gera gráficos de Intensidade-Duração-Freqência e de Precipitação-Duração-Freqência
